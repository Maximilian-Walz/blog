      ...
      <KBarPositioner className="z-30 bg-black/50 p-4 backdrop-blur backdrop-filter">
        <KBarAnimator className="w-full max-w-xl">
          <div className="overflow-hidden rounded-2xl border border-gray-800 bg-gray-900">
            ...
            {isLoading && (
              <div className="block border-t border-gray-800 px-4 py-8 text-center text-gray-600">
                Loading...
              </div>
            )}
          </div>
      ...
      <KBarResults
        ...
        onRender={({ item, active }) => (
          <div>
            {typeof item === 'string' ? (
              <div className="pt-3">
                <div className="block border-t border-gray-800 px-4 pb-2 pt-6 text-xs font-semibold uppercase text-primary-600">
                  {item}
                </div>
              </div>
            ) : (
              <div
                className={`flex cursor-pointer justify-between px-4 py-2 ${
                  active ? 'bg-primary-600 text-gray-100' : 'bg-transparent text-gray-100'
                }`}
              >
